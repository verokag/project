  ...
  public ActionResult Registro() 
        {
            ...
            Marcas = MarcaProcesoObjeto.SeleccionarMarcas(new MarcaEntidad());

            ViewBag.Marcas = ConvertListToSelectList(Marcas);

            return View(Registros);

        }

        private static List<SelectListItem> ConvertListToSelectList(List<MarcaEntidad> ListaMarcas) 
        {
            List<SelectListItem> ItemsList = new List<SelectListItem>();

            foreach (MarcaEntidad Marca in ListaMarcas)
            {
                SelectListItem item = new SelectListItem(){ Text = Marca.Nombre, Value = Marca.MarcaId.ToString()};
                ItemsList.Add(item);
            }
